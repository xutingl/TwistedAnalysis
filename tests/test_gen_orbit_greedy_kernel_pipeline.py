...
def test_end_to_end_pipeline_2x4x4_ilp_literal_greedy_rejected(tmp_path):
    ...
    rounds are not symmetric across sources. The `--per-step-barrier` emitter
    bakes src=0's column-to-round mapping into per-step unrolled drain calls,
    which is wrong on every source != 0 for an asymmetric schedule. The CLI
    must reject this combination with a clear SystemExit, rather than silently
    emitting a wrong kernel. Without `--per-step-barrier` the default path
    only uses `dest_table_ref` and works fine for literal_greedy (covered by
    the parametrized end-to-end test above).
    """
    from pallas_kernel.gen_orbit_greedy_kernel import main
