...
| `orbit_block_seq` | **Window-model** scheduler: packs orbits into blocks of uniform size ≥ `w`, then orders the blocks by bottleneck-greedy so adjacent unions stay light. Requires `w`. Emits one orbit per round, so dest-table column `k` is orbit `k` for every source | Optimizes max whole-path edge load over any sliding window of `w` dest-table columns — the quantity that predicts wall-clock on the all-up-front kernel, where no barrier makes `round` observable (see `max_window_edge_load`). Lower bound `LB(w) = ceil(w·75/127)`. On loaded 8×4×4, `w=12` scores **7/11/17/31/60** at `w`=6/12/24/48/96 vs `orbit_greedy_full`'s 8/13/20/36/66 — **9–15% lower at every window**, no tuning-window overfit. A flat least-burstiness-next greedy was tried first and failed (beat orbitfull only at its tuning window, `greedy(W=24)` scored 22 vs orbitfull's 20 at W=24); the fix is structural — `orbit_pack`'s ragged bins let a 6-window straddle three bins (3+3+3=9), uniform blocks cap the straddle at two. Regenerate via `eval/regenerate_8x4x4_orbit_block_seq_kernels.sh`. **Model not yet validated as a quantitative wall-clock predictor** |
| `orbit_pack_shuffled` | **Negative control** for `orbit_pack`. Reproduces that packing's step count and per-step orbit counts exactly, but assigns orbits to steps at random (seeded), forfeiting the whole-path edge cap `c`. Requires `k`/`c`; optional `seed` | Holds fixed everything an `orbit_pack` win could otherwise be credited to — same 127 orbits, still atomic (so every step is a set of permutations: no incast, no idle devices), same T, same per-device DMA depth per step — and gives up only the FFD certification. Exists because the 2026-07 TPU v4 runs found `orbit_pack_k6c3` beating P2P *without* the barrier and at parity with `orbit_greedy_full`, with the barrier adding nothing: absent a barrier, T is not a physical quantity, and the schedule reaches hardware only as a destination ordering. Measuring shuffled-vs-certified separates orbit-atomicity from congestion control. Does NOT verify at `c` — read its achieved cap with `verify.max_step_edge_load` (or `scripts/schedule_stats.py`) and pass that as `--step-edge-cap`. Regenerate via `eval/regenerate_8x4x4_orbit_pack_shuffled_kernels.sh` |
...
- `scripts/` — reproducible CLIs:
  ...
  - `schedule_stats.py` — `(schedule JSON) → barrier steps, achieved whole-path edge cap, per-device DMA depth`. `--field edge_cap` prints a bare int for shell capture (used to pick `--step-edge-cap` for uncertified schedules).
